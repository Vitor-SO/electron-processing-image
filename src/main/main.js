...
ipcMain.on("download-image", (event, url, imageName) => {
  dialog.showSaveDialog({ defaultPath: imageName }).then((currentPath) => {
    ...
    setTimeout(() => {
      fs.writeFileSync(
        `./src/renderer/processImage/Images/${imageName}`,
        fs.readFileSync(currentPath.filePath)
      );
    }, 1500);
...
ipcMain.on("import-image", () => {
  dialog
    .showOpenDialog({ defaultPath: app.getPath("downloads") })
    .then((currentPath) => {
      const basename = path.basename(currentPath.filePaths[0]);
      fs.writeFileSync(
        `./src/renderer/processImage/Images/${basename}`,
        fs.readFileSync(currentPath.filePaths[0])
      );
    });
});
...
ipcMain.on("create-coreWindow", () => {
  ...
  ipcMain.on("btn-histogram", () => {
    dialog
      ...
      .then((currentPath) => {
        //call the negative function
        const filename = path.basename(currentPath.filePaths[0]);
        const pathHistogram = filename.split(".")[0];
        Transformations.histogram(currentPath.filePaths[0], pathHistogram);
      });
  });
  ...
  ipcMain.on("btn-robert", () => {
    dialog
      .showOpenDialog({ defaultPath: app.getPath("") })
      .then((currentPath) => {
        const filename = path.basename(currentPath.filePaths[0]);
