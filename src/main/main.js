...
ipcMain.on("original-image", () => {
  dialog
    .showOpenDialog({
      filters: [{ name: "Images", extensions: ["jpg", "png", "gif", ".tif"] }],
    })
    .then((currentPath) => {
      ...
      pathImage.forEach(function ({ url, name }) {
        const dataImage = fs
          .readFileSync(path.resolve(__dirname, "..", url))
          .toString("base64");
...
ipcMain.on("process-image", (event, message) => {
  ...
  const paths = {
    ...
    equalizacaoGlobal:
      "/Transformations/Equalizacao/EqualizacaoGlobal/equalizacao_global.py",
    equalizacaoLocal:
      "/Transformations/Equalizacao/EqualizacaoLocal/equalizacao_local.py",
